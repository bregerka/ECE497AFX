...
LFO_freq_Hz = 10;   % low-frequency oscillator rate (Hz) / 1Hz / 0.5 to 20Hz
...
a = 0.5;            %value between 0 and 1 that sets gain for LFO
...
% visualize one period of the waveform, comment out if using sine wave
%figure(1);
%plot(LFO);
%title('one period');

%% Create a signal source from this waveform, Comment out if not using sin
